...
int main(int argc, char **argv)
{
  ...
  auto segmentation_client = node->create_client<segmentation_srvs::srv::Segmentation>("human_mask");
  // cv bridge
    ...
    if (err == ERROR_CODE::SUCCESS)
    {
      ...
      if (bodies.is_new)
      {
        try
        {
          ...
          // convert cvImage to uint8_t using opencv
          cv::Mat image_req = cvImage.clone();
          // cv::imwrite("image_req.png", image_req);
          // get mask from service
          auto request = std::make_shared<segmentation_srvs::srv::Segmentation::Request>();
          std_msgs::msg::Header header;
          header.stamp = node->now();

          auto ros_image = cv_bridge::CvImage(header, sensor_msgs::image_encodings::RGB8, image_req);
          sensor_msgs::msg::Image img_msg;
          ros_image.toImageMsg(img_msg);

          request->req = img_msg;
          // //sanity check -> reconstruct image from cvImage
          // auto cvImage_reconstructed = cv_bridge::toCvCopy(ros_image, "bgr8")->image;
          // // to rgb
          // cv::imwrite("image_reconstructed.png", cvImage_reconstructed);
          ...
          while (rclcpp::ok() && result.wait_for(1s) == std::future_status::timeout)
          {
            ...
            std::cout << "Waiting for mask" << std::endl;
            count++;
            if (count > 4)
            {
              rclcpp::shutdown();
          ...
          cv::Mat mask = cv_bridge::toCvCopy(result.get()->res, "mono8")->image;
